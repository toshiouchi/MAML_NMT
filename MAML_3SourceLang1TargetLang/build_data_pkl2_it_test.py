...
def main(sentence_start, sentence_end ):
    #メタ学習する言語
    #m_langs = [ "de" ]
    m_langs = [ "it" ]
    #m_langs = [ "de", "fr" ]
    ...
    for i, m_lang in enumerate( m_langs ):
        ...
        for j, (source, target) in enumerate( zip( tmp_s, tmp_t ) ):
            ...
            if j > sentence_end:
                break
            if j >= sentence_start and j <= sentence_end:
                r = {}
                r['domain'] = i
    ...
    with open("data_test_it_unused.pkl", mode="wb") as f:
...
if __name__ == "__main__":
    ...
    sentence_start = 600000
    sentence_end = 600499
    
    main( sentence_start, sentence_end )
