...
def main(sentence_max):
    ...
    for i, m_lang in enumerate( m_langs ):
        ...
        for j, (source, target) in enumerate( zip( tmp_s, tmp_t ) ):
            ...
            if i == 2:
                r = {}
                r['domain'] = i
                source = tokenizer(source.lower(), word_to_id_s)
                target = tokenizer(target.lower(), word_to_id_t)
                r['source'] = source
                r['target'] = target
                reviews.append( r )
                r = {}
    ...
    point1 = len( reviews ) * 1998 // 2000
    point2 = len( reviews ) * 1999 // 2000
    ...
    # ファイルに pkl で書き込み。
    with open("data_train.pkl", mode="wb") as f:
        pickle.dump(data_train, f)   
    with open("data_val.pkl", mode="wb") as f:
        pickle.dump(data_val, f)   
    with open("data_test.pkl", mode="wb") as f:
...
if __name__ == "__main__":
    ...
    sentence_max = [ 1000000, 1000000, 500000 ]
